...
const typeOf = (url) => {
  ...
  if (["txt", "csv"].includes(extension))
    return [
      extension === "csv" ? "Hoja de cálculo CSV" : "Documento de texto",
      true,
    ];
          ...
          (item) =>
            `<div class="item">
              <div>
                ...
                <small>${esc(item.documentId)}</small>
                <small>${typeOf(item.fileUrl)[0]}${item.reason ? ` · ${esc(item.reason)}` : ""}</small>
              </div>
              <div>
                <span class="badge ${item.status}">${item.status}</span>
                <div class="actions">
                  <button class="primary" data-review="${item.documentId}">Revisar documento</button>
                  <button class="danger" data-delete="${item.documentId}">Eliminar</button>
                </div>
              </div>
            </div>`,
        )
...
function review(id) {
  ...
  document.querySelector("#previewFrame").src = inline
    ? selected.fileUrl
    : "about:blank";
  document.querySelector("#previewFallback").style.display = inline
    ? "none"
    : "grid";
  document.querySelector("#decisionActions").style.display =
    selected.status === "pending" ? "flex" : "none";
  preview.showModal();
}
  ...
  try {
    ...
    show(
      data.webhook.delivered
        ...
        : "Decisión guardada; webhook pendiente.",
    );
    await load();
  } catch (error) {
...
list.addEventListener("click", async (event) => {
  ...
  if (deleteButton && confirm("¿Eliminar esta solicitud de Sistema B?")) {
    const response = await fetch(`/documents/${deleteButton.dataset.delete}`, {
      method: "DELETE",
    });
...
document.querySelector("#rejectForm").onsubmit = (event) => {
  event.preventDefault();
  const value = reason.value.trim();
  if (value) {
    reject.close();
    decide("rejected", value);
  }
};
reason.oninput = () => {
